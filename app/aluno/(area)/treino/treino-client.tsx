...
export function TreinoClient({
  ...
  const respondido = estado !== null && !("error" in estado);
  const correta = respondido && "correta" in estado ? estado.correta : null;
  const altCorretaId = respondido && "alternativa_correta_id" in estado ? estado.alternativa_correta_id : null;

  // Pré-carrega alternativas da próxima questão
  useEffect(() => {
    const prox = questoes[idx + 1];
    if (prox && !altsMap[prox.id]) {
      getAlternativasQuestao(prox.id).then((a) =>
        setAltsMap((m) => ({ ...m, [prox.id]: a }))
      );
    }
  ...
  if (idx >= total) {
    return (
      <div className="rounded-xl border border-border bg-card p-12 text-center">
        ...
        <p className="text-muted-foreground mb-6">Você respondeu todas as {total} questões desta sessão.</p>
        <div className="flex justify-center gap-3">
          <Link href="/aluno/treino/dashboard" className="rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground">
            Ver meu desempenho
          </Link>
          <Link href="/aluno/treino" className="rounded-lg border border-border px-5 py-2.5 text-sm text-muted-foreground hover:text-foreground">
            Nova sessão
          </Link>
        </div>
      </div>
    ...
    <div>
      ...
      <div className="mb-4">
        <div className="h-1.5 rounded-full bg-card overflow-hidden mb-1.5">
          <div className="h-full rounded-full transition-all" style={{ width: `${(idx / total) * 100}%`, background: TEAL }} />
        </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6 mb-4">
        ...
        {/* Enunciado */}
        <p className="text-[15px] leading-relaxed text-foreground mb-4 whitespace-pre-wrap">{questao.enunciado}</p>

        {/* Imagem do enunciado */}
        {questao.imagem_url && (
          <div className="mb-4">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={questao.imagem_url} alt="Figura da questão" className="max-w-full rounded-lg border border-border" />
          </div>
        )}
          ...
          <div className="space-y-2.5 mb-5">
            {alts.length === 0 ? (
              ...
              alts.map((alt) => {
                ...
                const isErrada = respondido && alt.id === (estado as any)?.alternativa_id && !correta; // eslint-disable-line
                ...
                return (
                  <div key={alt.id} className={`rounded-lg border-2 transition-all ${isNeutra ? "border-border cursor-pointer hover:border-sky-400 hover:bg-sky-400/5" : isCorreta ? "border-emerald-500 bg-emerald-500/8" : isErrada ? "border-red-500 bg-red-500/8" : "border-border opacity-60"}`}>
                    <form action={action}>
                      ...
                      <button type="submit" disabled={respondido || isPending} className="flex items-start gap-3 w-full p-3 text-left">
                        ...
                        <div className="flex-1">
                          ...
                          {alt.imagem_url && (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={alt.imagem_url} alt={`Alternativa ${alt.letra}`} className="mt-2 max-w-xs rounded border border-border" />
                          )}
                        </div>
                      </button>
                    </form>
        ...
        {/* Feedback */}
        {respondido && (
          <div className={`flex items-center gap-2 rounded-lg border px-4 py-3 text-sm font-semibold mb-4 ${correta ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400" : "border-red-500/30 bg-red-500/10 text-red-400"}`}>
        ...
        {respondido && mostrarGabarito && (
          <div className="rounded-xl border border-border bg-background overflow-hidden mb-4">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-border">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={TEAL} strokeWidth="2"><path d="M9 12l2 2 4-4"/><circle cx="12" cy="12" r="10"/></svg>
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: TEAL }}>Gabarito</span>
            </div>
            <div className="p-5 space-y-4">
              {/* Resolução em texto */}
              {gabarito?.texto ? (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Resolução</p>
                  <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-wrap">{gabarito.texto}</p>
                </div>
              ) : (
                <p className="text-sm text-muted-foreground italic">Resolução em texto não disponível para esta questão.</p>
              )}
              {gabarito?.imagem_url && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={gabarito.imagem_url} alt="Resolução" className="max-w-full rounded-lg border border-border" />
              )}
              {/* Vídeo */}
              {questao.video_url && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Resolução em vídeo</p>
                  <a href={questao.video_url} target="_blank" rel="noopener noreferrer"
        ...
        <div className="flex gap-3">
          ...
          {respondido && (
            <button
              ...
              className="rounded-lg px-5 py-2 text-sm font-bold text-[#0f172a]"
              style={{ background: TEAL }}
            >
              {idx + 1 < total ? "Próxima questão →" : "Concluir sessão →"}
            </button>
          )}
        </div>
      </div>
