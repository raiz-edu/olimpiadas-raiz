...
function CardInstituicao({ inst }: { inst: InstituicaoIngresso }) {
  return (
    ...
    <div className="max-w-5xl space-y-8">
      <div>
        ...
        <p className="mt-2 rounded-lg border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          ...
          cada ciclo (geralmente entre outubro e janeiro) — confirme sempre no link oficial antes de
          orientar famílias e alunos.
        </p>
      </div>
        ...
        return (
          <section key={secao.categoria}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {itens.map((inst) => (
                <CardInstituicao key={inst.sigla} inst={inst} />
              ))}
            </div>
          </section>
        );
