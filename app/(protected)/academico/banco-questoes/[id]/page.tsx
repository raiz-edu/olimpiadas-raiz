    ...
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span className="text-foreground">
          {OLIMPIADA_LABEL[questao.olimpiada]} · {questao.fase}ª Fase · {questao.ano} · Q{questao.numero}
        </span>
      </div>
      ...
      {total > 0 && (
        <div className="flex gap-4 rounded-xl border border-border bg-card p-4 text-sm">
          <div className="text-center"><div className="text-xl font-bold text-foreground">{total}</div><div className="text-xs text-muted-foreground">respostas</div></div>
          <div className="text-center"><div className={`text-xl font-bold ${pct! >= 60 ? "text-emerald-400" : "text-amber-400"}`}>{pct}%</div><div className="text-xs text-muted-foreground">acertos</div></div>
        </div>
      )}
      ...
      <section className="rounded-xl border border-border bg-card p-6">
        ...
        <SolucaoEditor questaoId={id} solucao={solucao} />
      </section>
    </div>
  );
}
