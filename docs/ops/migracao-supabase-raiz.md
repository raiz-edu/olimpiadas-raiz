...
- **Segredos**: `olimpiadas-raiz/prd/runtime` no Secrets Manager precisa ganhar `OPENAI_API_KEY` e `CREDENCIAIS_MASTER_KEY`; o workflow só injeta as chaves listadas em `aws-deploy.yml` (já incluídas no PR). Enquanto `CREDENCIAIS_MASTER_KEY` não existir, a cifra usa uma chave **derivada de `SESSION_SIGNING_SECRET`** (HKDF) — a tela de credenciais funciona, e as chaves gravadas continuam legíveis depois que a chave própria for definida.
